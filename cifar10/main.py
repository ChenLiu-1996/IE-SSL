...
class IESSLTransform:

    def __init__(self):
        self.is_odd = True  # flag to identify adjacent pairs.
        self.seed1 = 0
        self.seed2 = 1
    ...
    def __call__(self, x):
        if self.is_odd:
            ...
            aug1 = single_transform(self.transform(x))
            random.seed(self.seed2)
            torch.manual_seed(self.seed2)
            aug2 = single_transform(self.transform(x))
            self.is_odd = False
        else:
...
def rotate_images(images):
    ...
    rotated_images = torch.zeros(
        [n_rot_images, images.shape[1], images.shape[2],
         images.shape[3]]).cuda()
...
def adjust_learning_rate(epochs, warmup_epochs, base_lr, optimizer, loader,
                         step):
...
def ssl_loop(args, encoder=None):
    if args.checkpoint_path:
        ...
        saved_dict = torch.load(os.path.join(
            args.checkpoint_path))['state_dict']
        main_branch.load_state_dict(saved_dict)
        file_to_update = open(
            os.path.join(args.path_dir, 'train_and_eval.log'), 'a')
    ...
    file_to_update = open(os.path.join(args.path_dir, 'train_and_eval.log'),
                          'w')

    # dataset
    if args.iessl_edim == 0:
        train_loader = torch.utils.data.DataLoader(
            dataset=torchvision.datasets.CIFAR10(
                '../data',
                train=True,
                transform=ContrastiveLearningTransform(),
                download=True),
            shuffle=True,
            batch_size=args.bsz,
            pin_memory=True,
    ...
    if args.loss == 'simsiam':
        dim_proj = [int(x) for x in args.dim_proj.split(',')]
        predictor = PredictionMLP(dim_proj[1], args.dim_pred,
                                  dim_proj[1]).cuda()

    # optimization
    optimizer = torch.optim.SGD(main_branch.parameters(),
                                momentum=0.9,
                                lr=args.lr * args.bsz / 256,
                                weight_decay=args.wd)

    if args.loss == 'simsiam':
        pred_optimizer = torch.optim.SGD(predictor.parameters(),
                                         momentum=0.9,
                                         lr=args.lr * args.bsz / 256,
                                         weight_decay=args.wd)
    ...
    for e in range(1, args.epochs + 1):
        ...
        for it, (inputs, y) in enumerate(train_loader,
            ...
            def forward_step():
                ...
                # (Ours) Use Invariance-Equivariance SSL.
                loss_equivariance = 0
                if args.iessl_edim > 0:
                    invariance_dim = z1.shape[1] - args.iessl_edim
                    assert args.iessl_edim >= 0 and invariance_dim >= 0, \
                    ...
                    if invariance_dim > 0:
                        ...
                        z1_full, z2_full = z1, z2
                        z1 = z1_full[:, :invariance_dim]
                        z2 = z2_full[:, :invariance_dim]
                        # The difference in each pair of consecutive data samples shall be similar.
                        diff = z1_full[:,
                                       invariance_dim:] - z2_full[:,
                                                                  invariance_dim:]
                        equivariance_vec1 = diff[::2, :]
                        equivariance_vec2 = diff[1::2, :]
                        loss_equivariance = info_nce_loss(
                            equivariance_vec1, equivariance_vec2)

                # forward pass
                if args.loss == 'simclr':
                    loss_invariance = info_nce_loss(
                        z1, z2) / 2 + info_nce_loss(z2, z1) / 2
                elif args.loss == 'simsiam':
                    p1 = predictor(z1)
                    p2 = predictor(z2)
                    loss_invariance = negative_cosine_similarity_loss(
                        p1, z2) / 2 + negative_cosine_similarity_loss(p2,
                                                                      z1) / 2
                else:
                    raise

                loss = loss_invariance + loss_equivariance

                # Using Equivariance SSL.
                if args.essl_lmbd > 0:
                    assert args.iessl_edim == 0, \
                    'E-SSL and IE-SSL shall not be used at the same time!'
        ...
        line_to_print = (f'epoch: {e} | knn_acc: {knn_acc:.3f} | '
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss',
                        default='simclr',
                        type=str,
                        choices=['simclr', 'simsiam'])
    parser.add_argument('--save_every', default=50, type=int)
    parser.add_argument('--warmup_epochs', default=10, type=int)
    parser.add_argument('--path_dir', default='../experiment', type=str)
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument('--essl-lmbd', default=0.0, type=float)
    parser.add_argument('--iessl-edim', default=0, type=int)
